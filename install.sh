...
echo "🚀 Запуск установки Discord Бота (v5.2 - Syntax Fix)..."
...
# ==============================================================================
# ВЕБ-ПАНЕЛЬ (ИСПРАВЛЕННАЯ ВЕРСИЯ БЕЗ ВЛОЖЕННЫХ ШАБЛОННЫХ СТРОК)
# ==============================================================================
cat << 'MAINEOF' > web/server.js
...
app.get('/dashboard', auth, (req, res) => {
    ...
    const dashboardHTML = '<nav class="bg-card border-b border-border px-6 py-4 flex justify-between items-center sticky top-0 z-10">' +
        ...
        '<div class="flex items-center gap-4">' + statusHtml +
        ...
        '  roles.forEach(function(r) { optionsHTML += \'<option value="\' + r.id + \'">\' + r.name + \'</option>\'; });' +
        '  select.innerHTML = optionsHTML;' +
        '  var itemsRes = await fetch("/api/shop");' +
        '  var items = await itemsRes.json();' +
        '  var list = document.getElementById("shopList");' +
        '  if (items.length === 0) { list.innerHTML = \'<p class="text-muted text-center py-8">Магазин пуст</p>\'; return; }' +
        '  var itemsHTML = "";' +
        '  items.forEach(function(item) {' +
        '    var role = roles.find(function(r) { return r.id === item.role_id; });' +
        '    var roleName = role ? role.name : "Нет";' +
        '    var img = item.image_url ? \'<img src="\' + item.image_url + \'" class="w-10 h-10 rounded object-cover">\' : \'<div class="w-10 h-10 rounded bg-zinc-800 flex items-center justify-center"><i data-lucide="package" class="icon text-muted"></i></div>\';' +
    ...
    res.send(layout('Панель управления', dashboardHTML));
});
